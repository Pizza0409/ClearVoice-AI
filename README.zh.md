...
[🚀 立即使用 (Web Demo)](https://clear-voice-ai.vercel.app/) | [📖 English README (default)](README.md)
...
```text
aiNoiseFilter/
├── README.md              # 英文說明（預設，適合 GitHub 首頁）
...
3. 在 Vercel 環境變數中設定：
   * `NEXT_PUBLIC_API_URL`：指向您的 HF Space URL
   * `NEXT_PUBLIC_SITE_URL`：您的 Vercel 正式網址（例如 `https://clear-voice-ai.vercel.app`），以利 SEO OG 圖與 `sitemap` / `robots`
